...
export async function reviewPullRequest(
  ...
  try {
    ...
    if (!canReview) {
      throw new Error(
        "Review limit reached. Please upgrade your subscription to continue."
      );
    }
    ...
    try {
          ...
          data: {
            ...
            review: `Error:${
              error instanceof Error
                ? error.message
                : "Unknown error occurred while requesting review."
            }`,
            status: "failed",
          },
        });
      ...
      console.error("Failed to log review error to database:", dberror);
    }
  }
}
